...
public class MatchMultipleImages {
	...
	private void matchMultipleImagesInMemory() {
		...
		matcher.addImages(copyright,highQuality,lowQuality,thumbnail);
		...
		System.out.println("\nImages Similar to high quality: ");
		
		//Print out results
		similarImages.forEach(result ->{
			System.out.printf("\nDistance: %.3f Image: @%s%n",result.distance,System.identityHashCode(result.value));
		});
	...
	private void loadImages() {
		try {
			ballon = ImageIO.read(getClass().getResourceAsStream("images/lowQuality.jpg"));
